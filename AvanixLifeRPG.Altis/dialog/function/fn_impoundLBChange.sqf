...
_retrievePrice = [_className,__GETC__(avanix_impound_prices)] call TON_fnc_index;
_retrievePrice = if(_retrievePrice == -1) then {1000} else {(__GETC__(avanix_impound_prices) select _retrievePrice) select 1;};
