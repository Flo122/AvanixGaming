...
{
	_vehicleInfo = [_x select 2] call avanix_fnc_fetchVehInfo;
...
/*
{
	...
	_price = [_x select 2,__GETC__(avanix_garage_prices)] call TON_fnc_index;
	_sPrice = [_x select 2,__GETC__(avanix_garage_sell)] call TON_fnc_index;
	if(_price == -1) then {_price = 1000;} else {_price = (__GETC__(avanix_garage_prices) select _price) select 1;};
	if(_sPrice == -1) then {_sPrice = 1500;} else {_sPrice = (__GETC__(avanix_garage_sell) select _sPrice) select 1;};
	
	_control lbAdd format["%1 - %4 Fee: %2€ || %5: %3€", _displayName,[_price] call avanix_fnc_numberText,[_sPrice] call avanix_fnc_numberText,localize "STR_Garage_SFee",localize "STR_Garage_SellPrice"];
