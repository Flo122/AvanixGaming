...
_control = ((findDisplay 39400) displayCtrl 39402);
{
	if(alive _x) then {
		...
		_ind = [_className,(call avanix_garage_sell)] call TON_fnc_index;
		
		if(_ind != -1 && count crew _x == 0) then {
			_price = ((call avanix_garage_sell) select _ind) select 1;
