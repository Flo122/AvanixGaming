...
if(!dialog) then
{
	if(!(createDialog "avanix_impound_menu")) exitWith {};
...
for "_i" from 0 to (count avanix_vehicles)-1 do
{
	_veh = avanix_vehicles select _i;
	if(_veh distance impound_obj < 50) then
	{
		_price = [_veh] call avanix_fnc_impoundPrice;
		switch (true) do
		{
		...
		_color = [(typeOf _veh),(_veh getVariable "avanix_VEH_color")] call avanix_fnc_vehicleColorStr;
		...
		_cars lbAdd format["%1 %2 - Cost: %3€",_name,_text,[_price] call avanix_fnc_numberText];
