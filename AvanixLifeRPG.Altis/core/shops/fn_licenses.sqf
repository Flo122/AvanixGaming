...

{
	...
	_longname = ([_shortname, 0] call avanix_fnc_licenseType) select 0;
	_displayname = ([_shortname, 0] call avanix_fnc_licenseType) select 1;
	_price = [_shortname] call avanix_fnc_licensePrice;
	_hasLicense = missionNamespace getVariable _longname;

	if(!_hasLicense) then
	{
		//_listbox lbAdd format["%1 (%2€)", _displayname, _price];
		_listbox lbAdd format["%1 (%2€)", _displayname,[_price] call avanix_fnc_numberText];
