...
_shop_data = [avanix_shop_type] call avanix_fnc_virt_shops;
ctrlSetText[2403,format["%1", _shop_data select 0]];

{
	_name = [([_x,0] call avanix_fnc_varHandle)] call avanix_fnc_vartostr;
	_index = [_x,__GETC__(buy_array)] call TON_fnc_index;
	if(_index != -1) then
	{
		_price = (__GETC__(buy_array) select _index) select 1;
		_item_list lbAdd format["%1  (%2€)",_name,[_price] call avanix_fnc_numberText];
