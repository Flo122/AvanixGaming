...
if(_veh isKindOf "House_F") then {
	private["_mWeight"];
	_mWeight = 0;
	{_mWeight = _mWeight + ([(typeOf _x)] call avanix_fnc_vehicleWeightCfg);} foreach (_veh getVariable["containers",[]]);
	_veh_data = [_mWeight,(_veh getVariable["Trunk",[[],0]]) select 1];
} else {
	_veh_data = [_veh] call avanix_fnc_vehicleWeight;
};
...
//Player Inventory Items
{
		...
		else
	{
		...
		{
			_pInv lbAdd format["%1€",[avanix_cash] call avanix_fnc_numberText];
...
//Trunk Inventory Items
{
		...
		else
	{
		...
		if(_val > 0) then
		{
			_tInv lbAdd format["%1€",[_val] call avanix_fnc_numberText];
