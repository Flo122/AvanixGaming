...
_canUseControls = {
	...
	if((player getVariable["restrained",FALSE]) OR (player getVariable["Escorting",FALSE]) OR (player getVariable["transporting",FALSE]) OR (avanix_is_arrested) OR (avanix_istazed)) then {false} else {true};
};
...
while {true} do
{
	...
	_abortButton = (findDisplay 49) displayCtrl 104;
	_abortButton buttonSetAction "[[player],""TON_fnc_cleanupRequest"",false,false] spawn avanix_fnc_MP";
