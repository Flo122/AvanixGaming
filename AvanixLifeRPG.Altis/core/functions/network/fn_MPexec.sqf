/*
	...
	Parameter(s):
	_this select 0: STRING - Packet variable name (always "avanix_fnc_MP_packet")
	_this select 1: ARRAY - Packet value (sent by avanix_fnc_MP function; see it's description for more details)
...
if(!(["avanix_fnc_",_functionName] call BIS_fnc_inString) && {!(["SPY_fnc_",_functionName] call BIS_fnc_inString)} && {!(["DB_fnc_",_functionName] call BIS_fnc_inString)} && {!(["TON_fnc_",_functionName] call BIS_fnc_inString)} &&
	...
	if(toLower(_functionName) == "spy_fnc_cookiejar") exitWith {
		...
		if(_reportUID != _callerUID) exitWith {
			if(isServer && _mode == 0) then {
				...
				[[_callerName,"False reporting to SpyGlass (cheater)"],"SPY_fnc_notifyAdmins",true,false] spawn avanix_fnc_MP;
			};
			_exitScope = true;
		};
	};
...
if (ismultiplayer && _mode == 0) then {
	if (isserver) then {
		if (typename _target == typename []) then {
			...
			//--- Multi execution
			{
				[_varName,[_mode,_params,_functionName,_x,_isPersistent,_isCall,_callerName,_callerUID]] call avanix_fnc_MPexec;
			} foreach _target;
		} else {
			...
			avanix_fnc_MP_packet = [1,_params,_functionName,_target,_isPersistent,_isCall,"__SERVER__","__SERVER__"];

			//--- Send to clients
			if (_ownerID < 0) then {
				//--- Everyone
				publicvariable "avanix_fnc_MP_packet";
			} else {
				if (_ownerID != _serverID) then {
					//--- Client
					_ownerID publicvariableclient "avanix_fnc_MP_packet";
				};
			};

			//--- Server execution (for all or server only)
			if (_ownerID < 0 || _ownerID == _serverID) then {
				["avanix_fnc_MP_packet",avanix_fnc_MP_packet] spawn avanix_fnc_MPexec;
			};
