...

{
	...
	switch(_item) do
	{
		case "avanix_inv_water":
		{
			if(_value > 0) then
			{
				_var = [_item,1] call avanix_fnc_varHandle;
				_pos = _unit modelToWorld[0,3,0];
				_pos = [_pos select 0, _pos select 1, 0];
				_obj = "Land_BottlePlastic_V1_F" createVehicle _pos;
				//_obj setVariable["water",_value,true];
				[[_obj],"avanix_fnc_simDisable",nil,true] spawn avanix_fnc_MP;
				_obj setPos _pos;
...
} foreach (avanix_inv_items + ["avanix_cash"]);