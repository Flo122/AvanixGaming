...
if(count _itemArray == 0) exitWith
{
    switch(playerSide) do {
        case west: {
            [] call avanix_fnc_copLoadout;
        };
        
        case civilian: {
            [] call avanix_fnc_civLoadout;
        };

        case independent: {
            [] call avanix_fnc_medicLoadout;
        };
    };
};
...
if(_goggles != "") then {_handle = [_goggles,true,false,false,false] spawn avanix_fnc_handleItem; waitUntil {scriptDone _handle};};
if(_headgear != "") then {_handle = [_headgear,true,false,false,false] spawn avanix_fnc_handleItem; waitUntil {scriptDone _handle};};
if(_uniform != "") then {_handle = [_uniform,true,false,false,false] spawn avanix_fnc_handleItem; waitUntil {scriptDone _handle};};
if(_vest != "") then {_handle = [_vest,true,false,false,false] spawn avanix_fnc_handleItem; waitUntil {scriptDone _handle};};
if(_backpack != "") then {_handle = [_backpack,true,false,false,false] spawn avanix_fnc_handleItem; waitUntil {scriptDone _handle};};
{_handle = [_x,true,false,false,false] spawn avanix_fnc_handleItem; waitUntil {scriptDone _handle};} foreach _items;
...
avanix_maxWeight = 100;
{
    _item = [_x,1] call avanix_fnc_varHandle;
    [true,_item,1] call avanix_fnc_handleInv;
} foreach (_yItems);
avanix_maxWeight = 24;

//Primary & Secondary (Handgun) should be added last as magazines do not automatically load into the gun.
if(_prim != "") then {_handle = [_prim,true,false,false,false] spawn avanix_fnc_handleItem; waitUntil {scriptDone _handle};};
if(_seco != "") then {_handle = [_seco,true,false,false,false] spawn avanix_fnc_handleItem; waitUntil {scriptDone _handle};};
...
if(playerSide == independent && {uniform player == "U_Rangemaster"}) then {
	[[player,0,"textures\medic_uniform.jpg"],"avanix_fnc_setTexture",true,false] spawn avanix_fnc_MP;
};