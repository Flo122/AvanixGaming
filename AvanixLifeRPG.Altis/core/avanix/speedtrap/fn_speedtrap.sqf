...
while{true} do
{
    if((player == vehicle player) || (player != driver (vehicle player))) then
    {
        ...
        waitUntil {uisleep 1; player != vehicle player && player == driver (vehicle player)};
    ...
    //diag_log format ["%1 ::Avanix:: speedtrap _speed=%2 traps=%3", time, _speed, _speedtraps];
    
    //detector
    _has_radar_detector = 0;
    _var_speedtrapdetector = ["speedtrapdetector",0] call avanix_fnc_varHandle;
    ...
    if((count _speedtraps) > 0 && _has_radar_detector == 1 && _speed > 0) then
    {
        ...
        if( (_last_detector_time == 0) || ((_last_detector_time + _blocktime_detector) < time) ) then
        {
            ...
            if((count _trap_object) > 0) then
            {
                ...
                [["", _trap_object select 0, "Radarfalle", time],"avanix_fnc_addTracavanixInfosToPlayers",player,false] spawn avanix_fnc_MP;
    ...
    
    {
        ...
        //diag_log format ["%1 ::Avanix:: speedtrap _trap_object=%2 _speedlimit=%3", time, _trap_object, _speedlimit];
        ...
        if(_speedlimit > 0) then
        {
            ...
            if(_distance < _trap_distance && _speed > (_speedlimit + _trap_speed_tolerance)) then
            {
                ...
                if(!_visible) then
                {
                    ...
                    //diag_log format ["%1 ::Avanix:: speedtrap _distance=%2 _speed=%3", time, _distance, _speed];
                    ...
                    if( (_last_traptime == 0) || ((_last_traptime + _blocktime_trap) < time) ) then
                    {
                        ...
                        //[[getPlayerUID player, name player, "customtype", 10000, _customType],"avanix_fnc_wantedAdd",false,false] spawn avanix_fnc_MP;
                        _trapped_infos = [];
                        ...
                        [_trapped_infos, _trap_object] spawn avanix_fnc_trapped;
                    };
                };
            };
        };
