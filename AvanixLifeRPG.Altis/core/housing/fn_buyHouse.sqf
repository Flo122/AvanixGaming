...
[
	...
	[(_houseCfg select 0)] call avanix_fnc_numberText,
	(_houseCfg select 1)],localize "STR_House_Purchase",localize "STR_Global_Buy",localize "STR_Global_Cancel"
] call BIS_fnc_guiMessage;

if(_action) then 
{
	if(avanix_atmcash < (_houseCfg select 0)) exitWith {hint format [localize "STR_House_NotEnough"]};
	[[_uid,_house],"TON_fnc_addHouse",false,false] spawn avanix_fnc_MP;
	_house setVariable["house_owner",[_uid,profileName],true];
	_house setVariable["locked",true,true];
	_house setVariable["Trunk",[[],0],true];
	_house setVariable["containers",[],true];
	_house setVariable["uid",round(random 99999),true];
	_house setVariable["rented",0,true];
	_house setVariable["bought",1,true];
	avanix_atmcash = avanix_atmcash - (_houseCfg select 0);
