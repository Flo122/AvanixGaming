...
avanix_net_dropped = true;
titleText[localize "STR_NOTF_NetDrop","PLAIN"];
uisleep 5;
if(count _fish == 0) exitWith {titleText[localize "STR_NOTF_NetDropFail","PLAIN"]; avanix_net_dropped = false;};
{
	if(_x isKindOf "Fish_Base_F") then
	{
		...
		if(([true,_type,1] call avanix_fnc_handleInv)) then
		{
			deleteVehicle _x;
