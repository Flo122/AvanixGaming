...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	[[0,format["%1 dein %2 wird von der Polizei beschlagnahmt.",(_vehicleData select 0) select 1,_vehicleName]],"avanix_fnc_broadcast",true,false] spawn avanix_fnc_MP;
	...
	5 cutText ["","PLAIN"];
	if(player distance _vehicle > 10) exitWith {hint "Beschlagnahmen abgebrochen!"; avanix_action_inUse = false;};
	...
	if((count crew _vehicle) == 0) then
	{
		...
		[[_vehicle,true,player,true],"TON_fnc_vehicleStore",false,false] spawn avanix_fnc_MP;
		waitUntil {!avanix_impound_inuse};
		hint format["Du hast einen %1 beschlagnahmt. Du hast %2€ erhalten!",_type,_price];
		[[0,format["%1 hat %2's %3 beschlagnahmt",player getVariable["realname",name player],(_vehicleData select 0) select 1,_vehicleName]],"avanix_fnc_broadcast",true,false] spawn avanix_fnc_MP;
