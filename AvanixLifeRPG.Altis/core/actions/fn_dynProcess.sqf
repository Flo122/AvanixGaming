...
if (avanix_is_processing) exitWith {hint "Du verarbeitest bereits etwas!"};
...
if (!_lizenzBoolean) then {
	if (avanix_cash>=_lizenzKaufkosten) then {
		avanix_cash = avanix_cash - _lizenzKaufkosten;
		call compile format ["%1 = true",_lizenzBenoetigt];
		hint format ["Du hast eine Lizenz fuer %1€ gekauft und kannst nun verarbeiten!",_lizenzKaufkosten];
	} else {
		hint format ["Leider hast du nicht %1€ dabei, um eine Lizenz zu erwerben!",_lizenzKaufkosten];
		if (true) exitWith {avanix_is_processing = false;5 cutText ["","PLAIN"];_nah=true};
	};
};
if (_nah) exitWith {avanix_is_processing = false;5 cutText ["","PLAIN"]};
...
_nah = false;
{
	...
	call compile format ["itemAnzahlHat = avanix_inv_%1",_itemName];
	if (_itemAnzahlBenoetigt>itemAnzahlHat) then {_nah = true};
} forEach _itemsBenoetigtArray;
if (_nah) exitWith {avanix_is_processing = false;5 cutText ["","PLAIN"];hint "Du hast nicht genuegend Items dabei!"};
...
while{true} do
{
	...
	if(_cP >= 1) exitWith {};
	if(player distance _currentPos > 10) exitWith {hint "Du hast dich zu weit entfernt!";avanix_is_processing = false;5 cutText ["","PLAIN"];_nah=true};
};
if (_nah) exitWith {avanix_is_processing = false;5 cutText ["","PLAIN"]};
...
_itemHatAnzahlArray = [];
{
	_itemsBenoetigtAktuell = _x;
	_itemName = _itemsBenoetigtAktuell select 0;
	call compile format ["_itemHatAnzahlArray = _itemHatAnzahlArray + [avanix_inv_%1]",_itemName];
} forEach _itemsBenoetigtArray;
...
{
	_itemsBenoetigtAktuell = _x;
	[false,_itemsBenoetigtAktuell select 0,((_itemsBenoetigtAktuell select 1)*_anzahlMaxVerarbeiten)] call avanix_fnc_handleInv;
} forEach _itemsBenoetigtArray;
// ITEMS ADDEN
{
	[true,_x select 0,((_x select 1)*_anzahlMaxVerarbeiten)] call avanix_fnc_handleInv;
} forEach _produkt;
