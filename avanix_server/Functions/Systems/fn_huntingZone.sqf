...
if(!_bool && count avanix_animals_array == 0) exitWith {};
...
if(_bool) then
{
	for "_i" from 0 to (_total)-1 do
	{
		...
		hint format["%1\n%2",_type,_animal];
		avanix_animals_array pushBack _animal;
	};
	avanix_animals_spawned = true;
	systemChat str(avanix_animals_array);
}
	else
{
	{
	...
	} foreach avanix_animals_array;
	
	avanix_animals_array = [];
	avanix_animals_spawned = false;
};