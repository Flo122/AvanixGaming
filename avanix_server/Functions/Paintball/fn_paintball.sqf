...
if (pb_spielstatus==1) then {
	...
	if (_geld>=5000) then {
		...
		[[5,"Du wurdest erfolgreich für das kommende Spiel angemeldet!"],"avanix_fnc_pb_response",_uid,false] spawn avanix_fnc_MP;
	} else {
...
if (pb_spielstatus==2&&count pb_spieler>=2)||(count pb_spieler==pb_maxspieler) then {
	[[1,"Leider hat das Spiel bereits begonnen oder die Lobby ist bereits voll, versuche es später noch einmal!"],"avanix_fnc_pb_response",_uid,false] spawn avanix_fnc_MP;
};