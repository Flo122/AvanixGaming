...
fnc_setupFuncs = {
    //So if we wanted to transfer avanix_fnc_someFunc we would do "someFunc".
    avanix_publicVarList = ["fuelConfig","initFuelAction","diesel","fuelPrices","super","vehicleCheck"];
    ...
    if(!isServer) then {
        ...
        cutText["Loading server side functions, please wait","BLACK FADED"];
        {
            ...
            waitUntil {!(isNil format["avanix_fnc_%1",_x]);};
        } forEach avanix_publicVarList;
    } else {
        {
            //Compile it
            [format["avanix_fnc_%1",_x]] call fnc_compileFinal;
            //Send it over the net
            publicVariable format["avanix_fnc_%1",_x];
        } forEach avanix_publicVarList;
    };
};
