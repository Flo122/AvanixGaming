...
switch(true)do
{
	...
	case (_vehicle iskindof "C_Van_01_transport_F"): {_liter = 0.00900; _completeliter = 25;};
	case (_vehicle iskindof "C_Van_01_fuel_F"): {_liter = 0.00900; _completeliter = 25;};
	case (_vehicle iskindof "C_Van_01_box_F"): {_liter = 0.00900; _completeliter = 25;};
	case (_vehicle iskindof "C_Kart_01_Fuel_F"): {_liter = 0.00900; _completeliter = 25;};
	case (_vehicle iskindof "C_Kart_01_Red_F"): {_liter = 0.00900; _completeliter = 25;};
	case (_vehicle iskindof "C_Kart_01_Vrana_F"): {_liter = 0.00900; _completeliter = 25;};
...
_action = [
	format["Möchtest du dein Fahrzeug %1 voll betanken? Das Tanken kostet %2 € und wird von deinem Konto abgezogen. Sollte der Tankvorgang unterbrochen werden, wird kein Geld erstattet.",_vehicleName,[_cost] call avanix_fnc_numberText],
	"Betanken",
	"Ja",
	"Nein"
...
if(_action) then {
	...
	if(avanix_atmcash < _cost) exitWith {hint format["Du hast nicht genuegend Geld auf der Bank! Du benoetigst %1",[_cost] call avanix_fnc_numberText]; 5 cutText ["","PLAIN"];};
	avanix_atmcash = avanix_atmcash - _cost;
	...
	while {fuel _vehicle < 0.99} do 
	{
		...
		if(vehicle player != player) exitWith {if (true) exitWith {};};
	...
	if (round (fuel _vehicle) > 0.99) then {
		_vehicle setFuel 1;
		5 cutText ["","PLAIN"];
		hint format ["Dein Fahrzeug %1 wurde fuer %2€ vollgetankt. Wir wuenschen eine gute Weiterfahrt" , _vehicleName,[_cost] call avanix_fnc_numberText];
